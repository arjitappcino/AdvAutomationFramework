...
/**
 * This class consists of all the common methods required to perform actions on web application
 * @author  Kamil Parvez
 * @version 1.0
 */
...
//			String url = ORUtil.getConfigValue(strEnvURL);
	...
	public static void launchLinux(String strEnvURL){
		...
		try {
			...
			if (browserType.equalsIgnoreCase("firefox")){
			   ...
			   // options.addArguments(new String[] { "--proxy-server=" + proxyURL });
	...
	/**
	 * This method is used to check Strings is matches to given Regular Expression
	 * 
	 * @author  Kamil Parvez 
	...
	public static void webClick(String objName) throws NoSuchElementException{
		...
		try {
			...
			if(lwebElement==null)
			ReportUtil.reporterEvent("info", "Web Element [ " + objName + " ] NOT FOUND on UI" + captureScreenshotAsBase64());
		}
...
public static void webClickActionClass(WebElement objName){
	...
	public static void switchToFrameWithType(String frameType){
		try{
			...
			ReportUtil.reporterEvent("fail", "Frame with Type [ " + frameType + " ] NOT FOUND" + captureScreenshotAsBase64());
		}
	}
